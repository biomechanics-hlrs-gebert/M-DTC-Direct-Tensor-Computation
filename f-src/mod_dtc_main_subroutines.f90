...
REAL(rk) :: t_start, t_end, mem, t_duration
...
INTEGER(ik) :: domain_elems, ii, jj, kk, id, stat, &
    ...
    timestamp, macro_order, no_elem_nodes, no_lc, HU, lower_limit, upper_limit
...
CHARACTER(mcl) :: timer_name, domain_desc, part_desc, map_sgmnttn="", &
...
Type(tBranch), pointer :: boundary_branch, domain_branch, part_branch, result_branch
...
Real(rk),    Dimension(6,6) :: K_loc_08_reverted
Type(tVec), Dimension(24) :: FF_08

IF (rank_mpi == 0) CALL CPU_TIME(t_start)
...
if (TRIM(elt_micro) == "HEX08") then
    ...
    Do ii = 1, domain_elems
        ...
        IF (bin_sgmnttn == "N") THEN

            bone_adjusted = bone

            HU = part_branch%leaves(6)%p_int8(ii)

            ! HU Values as they represent water (~blood) and the most dense cortical bone (~2000)
            lower_limit = 0._rk
            upper_limit = 2000._rk
    
            IF ((HU > lower_limit) .AND. (HU < upper_limit)) THEN
    
                ! Linear map
                bone_adjusted%E = bone%E/upper_limit * REAL(HU,rk)

            ELSE IF (HU > upper_limit) THEN
                bone_adjusted%E = bone%E
                
            ELSE IF (HU < lower_limit) THEN
                bone_adjusted%E = 0._rk

            END IF 
            
            K_loc_08 = stiffness_matrix_brick_HEX08(bone_adjusted)
            
        ELSE ! (bin_sgmnttn == "N") THEN
            K_loc_08 = stiffness_matrix_brick_HEX08(bone)
        END IF 
...
!------------------------------------------------------------------------------
! Write the start time and the duration to file
!------------------------------------------------------------------------------
IF (rank_mpi == 0) THEN

    Call Search_branch("Results of domain "//domain_char, domain_tree, result_branch, success)
